export const tourData = [
  {
    id: 1,
    city: "new york",
    img: "./img/newyork.jpeg",
    name: "new york bridge tour",
    info:
      "New York 30M nüfusa sahip ABD'nin en kalabalık kentidir ve Kuzey Doğuda yer alır.Özgürlük Anıtı ise görülmeye değer en önemli turistik yapıdır."
  },
  {
    id: 2,
    city: "paris",
    img: "./img/paris.jpeg",
    name: "paris lights tour",
    info:
      "Paris Avrupa'nın moda merkezlerinden biri olarak tanımlanır. Şehirde Araplar ve Çinlilerde yoğun olarak yaşamaktadır. Eiffel Tower, Sacre Coeur, Notre Dame görülmeye değer yapılardır."
  },
  {
    id: 3,
    city: "london",
    img: "./img/london.jpeg",
    name: "london royal palace tour",
    info:
      "Londra İngiltere'nin başkentidir.Londra'daki saat kulesi ve yanındaki köprü muğazzamdır.Oxford gibi dünya klasındaki üniversitelere ev sahipliği yapar."
  },
  {
    id: 4,
    city: "tokyo",
    img: "./img/tokyo.jpeg",
    name: "tokyo sushi tour",
    info:
      "Tokyoyu sikiym!"
  },
  {
    id: 5,
    city: "jackson",
    img: "./img/jackson.jpg",
    name: "jackson hole",
    info:
      "Jackson Hole Wyoming eyaletindedir ve Teton Dağları dibindeki bir Town'dur.Yellowstone National Park'a 1 saat uzaklıkta ve Grand Teton National Park'a 15 dk uzaklıktadır.On numara yerdir. "
  },
  {
    id: 6,
    city: "lasvegas",
    img: "./img/lasvegas.jpg",
    name: "vegas baby",
    info:
      "What happens in Vegas, stays in Vegas!"
  },
  {
    id: 7,
    city: "sanfrancisco",
    img: "./img/sanfrancisco.jpg",
    name: "the city",
    info:
      "San Francisco büyüleyici bir şehirdir. Golden State Warriors bu şehrin takımıdır. Şehirde Alcatraz Hapishanesi ise çok önemli bir turistik noktadır. Lombard Streette arabanız ile menderesler çizebilirsiniz."
  }
];
